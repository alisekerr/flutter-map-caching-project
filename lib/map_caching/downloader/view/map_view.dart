...
import 'package:flutter_maps/map_caching/downloader/widgets/crosshairs.dart';
...
class _MapViewState extends State<MapView> {
                ...
                return Stack(
                  children: [
                    FlutterMap(
                      ...
                      children: [
                        _buildTileLayer(urlTemplate, context, generalState),
  ...
  TileLayer _buildTileLayer(
      String urlTemplate, BuildContext context, GeneralState generalState) {
    return TileLayer(
      urlTemplate: urlTemplate,
      maxZoom: 20,
      reset: context.read<GeneralBloc>().resetController.stream,
      keepBuffer: 5,
  ...
  void _updatePointLatLng() {
    ...
    context.read<MapBloc>().state.regionMode == RegionMode.circle
        ? context.read<MapBloc>().baseRegion = CircleRegion(_center!, _radius!)
        : context.read<MapBloc>().baseRegion = RectangleRegion(
            LatLngBounds(_coordsTopLeft, _coordsBottomRight),
          );
  }

  Future<void> _countTiles() async {
    if (context.read<MapBloc>().baseRegion != null) {
      //  context.read<MapBloc>().add(const RegionTilesSet(null));
      context.read<MapBloc>().add(
            RegionTilesSet(
              await FMTC.instance('').download.check(
                    context.read<MapBloc>().baseRegion!.toDownloadable(
